...
- [ g] Write an _npm script_ named _"start"_ that uses `node` to run the API server.
- [ g] Write an _npm script_ named _"server"_ that uses `nodemon` to run the API server.
- [ g] Install _nodemon_ as a development dependency that would not be used in production.
